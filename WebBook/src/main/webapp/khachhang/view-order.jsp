...
<body>
    ...
    <div class="select">
	  <select onchange="window.location.href = this.value;">
	    ...
	    <option value="khach-hang?action=view-order-all" ${viewOrderAll == true ? 'selected' : ''}>Tất cả</option>
	    <option value="khach-hang?action=view-order-confirmed" ${viewOrderOfUserConfirmed == true ? 'selected' : ''}>Đơn hàng đã xác nhận</option>
	    <option value="khach-hang?action=view-order-waiting" ${viewOrderOfUserWaitingConfirmed == true ? 'selected' : ''}>Đơn hàng đang chờ xác nhận</option>
	    <option value="khach-hang?action=view-order-cancelled" ${viewOrderOfUserCancelled == true ? 'selected' : ''}>Đơn hàng đã hủy</option>
	  </select>
	</div>
    </div>
    
    <div class="TatCa" style="${viewOrderAll == true ? 'display: block;' : 'display: none;'}">
    	<c:forEach items="${listAllOrderOfUser}" var="o">
    ...
    <div  class="DaXacNhan" style="${viewOrderOfUserConfirmed == true ? 'display: block;' : 'display: none;'}">
    	<c:forEach items="${listAllOrderOfUserConfirmed}" var="o">
    ...
    <div  class="ChuaXacNhan" style="${viewOrderOfUserWaitingConfirmed == true ? 'display: block;' : 'display: none;'}">
    	<c:forEach items="${listAllOrderOfUserWaitingConfirmed}" var="o">
	    ...
	    <div class="orderI">
		    ...
		    <h4>Ngày đặt: ${o.date_creat}</h4>
		 	<h4>Nơi nhận: ${o.deliveryaddress}</h4>
		 	<h4>Tổng tiền: ${o.total}</h4>
		 	<a href="#" class="button button2" onclick="showMess(${o.oid})">Hủy đặt hàng</a>
		 	
		</div>
		</c:forEach>
    </div>
    <div  class="DaHuy" style="${viewOrderOfUserCancelled == true ? 'display: block;' : 'display: none;'}">
    	<c:forEach items="${listAllOrderOfUserCancelled}" var="o">
...
function showMess(oid){
	...
	if(option === true){
		window.location.href = 'khach-hang?action=cancel-order&oid='+oid;
	}
}
</script>
</html>