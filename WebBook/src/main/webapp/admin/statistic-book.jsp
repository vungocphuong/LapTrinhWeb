        ...
        <header class="header">
            <div class="grid wide">
                ...
                <div class="header-with-search">
                    ...
                    <span style="color: white;font-size: 40px;">THỐNG KÊ SÁCH</span>
                </div>
            </div>    
        </header>
        
     <div class="mainStatistic">
     	<form action="<%=url%>/admin-ctr?action=statistic-book-by-revenue" method="post" enctype="multipart/form-data">
		...
		<div id="data">
			<table class="content-table">
			  ...
			  <tbody>
			   	...
			   	<c:forEach items="${listStaticBook}" var="o">
			   		<tr>
			    		<td>${o.bid}</td>
				      	<td>${o.title}</td>
				      	<td>${o.author}</td>
				      	...
				      	<td>${o.revenue}</td>
				      	<td><input type="button" class="button button4"  value="Chọn" onclick="window.location.href='<%=url %>/admin-ctr?action=view-detailed-book-statistics&start=${start}&end=${end}&bid=${o.bid}'"/></td>
				    </tr>
			    </c:forEach>
